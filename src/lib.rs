...
//     LambdaParticle, AlchemyCollider, LambdaCollisionOk, LambdaCollisionError,
// };
// type RustSoup = GenericSoup<LambdaParticle, AlchemyCollider, LambdaCollisionOk, LambdaCollisionError>;
...
// Bring in the concrete lambda types so we can fully specify Soup.
use crate::lambda::recursive::{
    AlchemyCollider, LambdaCollisionError, LambdaCollisionOk, LambdaParticle,
};

// Create a concrete alias for the generic Soup<P,C,T,E>.  Without this, the compiler
// complains about missing generic parameters and ambiguous new()/from_config() impls.
use crate::supercollider::Soup as GenericSoup;
type RustSoup =
...
#[derive(Debug, Clone)]
pub struct PyReactionError {
    // Store the kind you map to below rather than LambdaCollisionError itself.
...
// Map the recursive LambdaCollisionError into your public PyReactionError.
impl From<LambdaCollisionError> for PyReactionError {
    fn from(error: LambdaCollisionError) -> Self {
        let kind = match error {
            LambdaCollisionError::ExceedsReductionLimit => {
                ReactionErrorKind::ExceedsReductionLimit
            }
            LambdaCollisionError::NotEnoughExpressions => {
                ReactionErrorKind::NotEnoughExpressions
            }
            LambdaCollisionError::IsIdentity => ReactionErrorKind::IsIdentity,
            LambdaCollisionError::IsParent => ReactionErrorKind::IsParent,
            LambdaCollisionError::HasFreeVariables => ReactionErrorKind::HasFreeVariables,
...
impl PySoup {
    ...
    fn perturb(&mut self, expressions: Vec<String>) -> PyResult<()> {
    // Parse incoming strings to Terms…
    let terms = expressions
        .into_iter()
        .filter_map(|s| parse(&s, Classic).ok());

    // …and let LambdaSoup wrap them into LambdaParticle (recursive = false)
    ...
    /// Return the list of expressions in the soup in Church notation.
    fn expressions(&self) -> Vec<String> {
        self.inner
            .lambda_expressions()
            .map(|term| term.to_string())
    ...
    /// Return the set of unique expressions (each term converted to a string).
    fn unique_expressions(&self) -> Vec<String> {
        self.inner
            .unique_expressions()
            .into_iter()
    ...
    /// Return a (term, count) list giving population counts for each unique term.
    fn expression_counts(&self) -> Vec<(String, u32)> {
        self.inner
            .expression_counts()
            .into_iter()
    ...
    /// Return the Shannon-like entropy of the current population (base‑10 logarithm).
    fn population_entropy(&self) -> f32 {
        self.inner.population_entropy()
    }
}
...
impl PyBTreeGen {
    ...
    fn from_config(
        ...
        PyBTreeGen {
            inner: RustBTreeGen::from_config(&config),
        }
    }
...
impl PyFontanaGen {
    ...
    fn from_config(
        ...
        PyFontanaGen {
            inner: RustFontanaGen::from_config(&config),
        }
    }
